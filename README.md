...
鼠鬚管原本是「查词库」式输入法：输入拼音，按词频列出候选。这个 fork 在上面接入**本地神经网络联想**（[librime-ai-predict](https://github.com/wyjrichhh/librime-ai-predict)），结合你刚写下的上文，把你正在输入的拼音更准确地转成汉字，并把结果补进候选栏。

- **结合上文的拼音推理**：模型看的是「上文 + 当前拼音」，把你正在输入的拼音转成最可能的汉字——比如上文是「今天天气」、再输入 `zenmeyang` 时，它结合上下文直接给出「怎么样」，而不是词库按词频挑的默认候选。
- **长词是它的强项**：两个字的词语首选命中率约 92%，详见下方「准确率」。
